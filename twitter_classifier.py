...
def main_loop():
            ...
            for i in range(0, len(prediction_tweets)):
                if prediction_ids[i] not in tweetids:
                    if predictionsRF[i] or predictionsSVM[i]:
                        ...
                        stanford_tagged_tweets.append(t)

                        tag = nltk.pos_tag(nltk.word_tokenize(prediction_tweets[i]))
                        nltk_tagged_tweets.append(tag)
    ...
    #
    # tfidfizer = vectorizerTFIDF.fit_transform(training_tweets, training_targets)
    #
    ...
    #
    # supvecmach = LinearSVC().fit(tfidfizer, training_targets)
    # tst = vectorizerTFIDF.transform(test_tweets, test_targets)
    # print(supvecmach.score(tst, test_targets))
    # predicted_recruiting = 0
    # predicted_nonrecruiting = 0
    #
    # sql = "SELECT * FROM twitter_collect ORDER BY tweetid DESC limit 20000"
    #
    # with cnx.cursor() as cur:
    #     cur.execute(sql)
    #     results = cur.fetchall()
    #     for result in results:
    #         text = vectorizerTFIDF.transform([result['tweettext']])
    #         if supvecmach.predict(text):
    #             print(str(result['tweetid']) + ' ' + result['tweettext'])
...
# RF Nonstemming
# Correct recruiting =	0
# Correct nonrecruiting =	130
# False positives =	0
# Bad =		35